...
- `sequenceRecord()` rejects an array or a tuple at compile time. Its constraint mapped a tuple onto a tuple. So `sequenceRecord([ok(1), ok(2)])` compiled, and then it threw `InvalidResultStateError` with the message "Unreachable: Result is neither Ok nor Err". The constraint now also demands that a `length` key hold a `Result`, and the `length` of an array never does. A record can still hold a `Result` under the key `length`. A generic caller over `Record<string, Result<T, E>>` still compiles and keeps its return type. An argument whose type is a union of an array and a record also stops compiling. That call compiled before with the error type `never`, and it threw whenever the value was an array. An array still reaches the runtime check through JavaScript, `any`, a cast, or a type that widens it. An array is assignable to `Record<number, Result<T, E>>`, so a parameter of that type compiles and throws.
- `InvalidResultStateError` no longer starts its message with "Unreachable:". Every throw site is reachable, through JavaScript, `any`, a cast or a type that widens a value, so the prefix blamed the library for a mistake of the caller. The default message is now "Result is neither Ok nor Err". The constructor takes an optional second argument that replaces it. Three functions use it to name what they observed. `sequenceRecord()` reports "received an array; use sequence() for a list" and "property b holds undefined, not a Result". `collectFirstOkAsync()` and `collectFirstOkParallelAsync()` report "input 0 fulfilled with undefined, not a Result". The code `ERR_INVALID_RESULT_STATE` stays the same, so a caller that matches on the code sees no change.
