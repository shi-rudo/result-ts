...
| `MatchHandlerNotResultError` | `ResultTypeError` | `ERR_MATCH_HANDLER_NOT_RESULT` | A `.matchErrorToResult()` / `.matchErrorToResultAsync()` handler returns a naked value instead of `ok(...)` or `err(...)`. | `handlerName: string`, `returnedValue: unknown` |
