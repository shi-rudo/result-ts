...
describe('filterAsync', () => {
    it('keeps Ok when async predicate is true', async () => {
        const result = ok(42);
    ...
    it('turns Ok into Err when async predicate is false', async () => {
        const result = ok(30);
        const filtered = await result.pipeAsync(filterAsync(async (v) => v > 40, async () => 'too small'));
    ...
    it('waits for the async predicate', async () => {
        let called = false;
        const result = ok(50);
    ...
    it('waits for the async errorFn', async () => {
        let called = false;
        const result = ok(30);
