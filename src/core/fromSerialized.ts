 ...
 * ```
 *
 * `fromSerialized` only checks the discriminant (`_tag`), never the payload,
 * while its type parameters claim `T` and `E` for whatever `JSON.parse`
 * returned. It also throws for malformed input at exactly the boundary where
...
export function fromSerialized<T, E>(data: ResultType<T, E>): Result<T, E> {
    if (data !== null && typeof data === 'object') {
        ...
        if (data._tag === 'Ok') return ok<T, E>(data.value as T);
        if (data._tag === 'Err') return err<E, T>(data.error as E);
    }
